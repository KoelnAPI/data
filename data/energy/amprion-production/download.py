...
BASE_URL = "http://preview.amprion.de/applications/applicationfiles/netzlasterzeugung.php?"
...
def save(data, start_date, end_date):
    ...
    if not os.path.exists(yearfolder):
        os.mkdir(yearfolder)
    path = '%s/production_%s.csv' % (
        yearfolder, start_date.strftime("%Y%m"))
    with open(path, 'wb') as csvfile:
        writer = csv.writer(csvfile, quoting=csv.QUOTE_MINIMAL)
        writer.writerow(["date", "time_from", "time_to", "production"])
        rowcount = 0
