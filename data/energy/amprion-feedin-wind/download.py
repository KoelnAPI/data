...
BASE_URL = "http://preview.amprion.de/applications/applicationfiles/winddaten%s.php?"
BASE_URL += "mode=download&format=csv&start=%s&end=%s&lng=undefined&t=%s"


def download(start_date, end_date):
    suffix = ''
    if start_date > datetime(2008, 4, 1):
        suffix = '2'
    now = datetime.now().strftime("%s")
    start = start_date.strftime("%d.%m.%Y")
    end = end_date.strftime("%d.%m.%Y")
    url = BASE_URL % (suffix, start, end, now)
    r = requests.get(url)
